...
public interface Settings {
	...
	/**
	 * Check if a specific value exists. A value exists if {@link #get(String)} returns a non-null value.
	 *
	...
	default String get( String key, Object defaultValue ) {
		return get( key, String.class, String.valueOf( defaultValue ) );
	}

	/**
	 * Get a value from the settings node. This method is useful for specifying that the return type be a simple data type.
	 * <p>
	 * Example:
	 * <pre>
	 ...
	 * @return The setting value
	 */
	default <T> T get( String key, TypeReference<T> type ) {
		return get( key, type, null );
	}
	...
	/**
	 * Remove a value from the settings node. This is the same as setting the value to null.
	 *
	 * @param key The value key
	 */
